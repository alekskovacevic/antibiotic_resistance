...
parameters_10 <- c(betaS = 0.042,    # pneumococcal transmission rate 0.062
...
p_abx_10 = 0.10                  # proportion of infected individuals getting abx prophylaxis
psummer_10 = 1e-06              # summer invasion rate
pwinter_10 = 2.5e-06            # winter invasion rate
a_lockdown_10 = 0.77            # reduction in the nb of abx prescriptions 
covid_transm_lockdown_10 = 0.23  # reduction of the covid transmission during lockdown
IPD_risk_lockdown_10 = 0.2      # risk of IPD during lockdown 
IPD_risk_rest_10 = 0.4          # risk of IPD during the rest of the year 
...
p_10$p_inv_10 <- ifelse((p_10$times > 1089 & p_10$times < 1271), (psummer_10), (pwinter_10)) 
invasion_10 <- approxfun(p_10)       #Create the interpolating function
invasion_10(seq(from = 1, to = 3650, by = 1))
...
inputrisk_10(seq(from = 1, to = 3650, by = 1))
...
model_10 <- function(time, state, parameters) {  
  with(as.list(c(state, parameters)), { 
    ...
    lambdaXSR <- ((1-psingle)*(betaS*thetab_10*ps + betaS*thetab_10*fit*(1-ps))*2*q*(Ssr+Ssra) + 
                    ...
                    (1-psingle)*(betaS*thetab_10*ps + betaS*thetab_10*fit*(1-ps))*2*q*(Isr+Isra+Israbx) +
                    (1-psingle)*(betaS*thetab_10*ps + betaS*thetab_10*fit*(1-ps))*2*q*(Rsr+Rsra+Rsrabx))/N
    
    lambdaXSS <- ((1-psingle)*betaS*thetab_10*2*q*(Sss+Sssa) + 
                    (1-psingle)*betaS*thetab_10*2*q*(Ess+Essa) + 
                    (1-psingle)*betaS*thetab_10*2*q*(Iss+Issa+Issabx) + 
                    (1-psingle)*betaS*thetab_10*2*q*(Rss+Rssa+Rssabx))/N
    
    lambdaXRR <- ((1-psingle)*betaS*thetab_10*fit*2*q*(Srr+Srra) + 
                    (1-psingle)*betaS*thetab_10*fit*2*q*(Err+Erra) + 
                    (1-psingle)*betaS*thetab_10*fit*2*q*(Irr+Irra+Irrabx) + 
                    (1-psingle)*betaS*thetab_10*fit*2*q*(Rrr+Rrra+Rrrabx))/N
    ...
    dI <- -lambdaXS*I - lambdaXR*I - lambdaXSR*I - lambdaXSS*I - lambdaXRR*I + 
      ...
      (1-p_abx_10)*tau*a_10*I + r*Ia + alpha*E + covid - gammaC*I - p_abx_10*I + 
      gammaR*Irabx + gammaR*Irrabx
    
    dIs <- lambdaXS*I - gammaS*Is - k*lambdaS*Is - k*lambdaR*Is - (1-p_abx_10)*tau*a_10*Is + alpha*Es - gammaC*Is - p_abx_10*Is
    dIr <- lambdaXR*I - gammaR*Ir - k*lambdaS*Ir - k*lambdaR*Ir + (gammaS+w)*Isra - (1-p_abx_10)*tau*a_10*Ir + r*Ira + alpha*Er - gammaC*Ir - p_abx_10*Ir 
    
    dIsr <- lambdaXSR*I - gammaR*Isr + k*lambdaR*Is + k*lambdaS*Ir - 
      k*c*lambdaS*Isr - k*c*lambdaR*Isr + 2*k*c*lambdaR*Iss + 2*k*c*lambdaS*Irr - 
      (1-p_abx_10)*tau*a_10*Isr + alpha*Esr - gammaC*Isr - p_abx_10*Isr 
    
    dIss <- lambdaXSS*I - gammaS*Iss + k*lambdaS*Is + k*c*lambdaS*Isr - 2*k*c*lambdaR*Iss - 
      (1-p_abx_10)*tau*a_10*Iss + alpha*Ess - gammaC*Iss - p_abx_10*Iss 
    
    dIrr <- lambdaXRR*I - gammaR*Irr + k*lambdaR*Ir + k*c*lambdaR*Isr - 2*k*c*lambdaS*Irr - 
      (1-p_abx_10)*tau*a_10*Irr + r*Irra + alpha*Err - gammaC*Irr - p_abx_10*Irr 
    
    # Antibiotic exposed compartments:
    dIa <- -lambdaXR*Ia - lambdaXRR*Ia + (1-p_abx_10)*tau*a_10*I - r*Ia + alpha*Ea - gammaC*Ia 
    dIsa <- -(gammaS+w)*Isa - k*lambdaR*Isa + (1-p_abx_10)*tau*a_10*Is + alpha*Esa - gammaC*Isa
    dIra <- lambdaXR*Ia - gammaR*Ira + (1-p_abx_10)*tau*a_10*Ir - r*Ira + alpha*Era - gammaC*Ira
    dIsra <- -(gammaS+w)*Isra + k*lambdaR*Isa + 2*k*c*lambdaR*Issa + (1-p_abx_10)*tau*a_10*Isr + alpha*Esra - gammaC*Isra 
    dIssa <- -(gammaS+w)*Issa - 2*k*c*lambdaR*Issa + (1-p_abx_10)*tau*a_10*Iss + alpha*Essa - gammaC*Issa  
    dIrra <-  lambdaXRR*Ia + (1-p_abx_10)*tau*a_10*Irr - r*Irra - gammaR*Irra + alpha*Erra - gammaC*Irra 
    
    # Additional antibiotic (abx) exposed compartments:
    dIabx <- p_abx_10*I - gammaC*Iabx - lambdaXR*Iabx - lambdaXRR*Iabx + (gammaS+w)*Isabx + (gammaS+w)*Issabx 
    dIsabx <- p_abx_10*Is - gammaC*Isabx - (gammaS+w)*Isabx - k*lambdaR*Isabx   
    dIrabx <- p_abx_10*Ir - gammaC*Irabx + lambdaXR*Iabx - gammaR*Irabx + (gammaS+w)*Israbx 
    dIsrabx <- p_abx_10*Isr - gammaC*Israbx - (gammaS+w)*Israbx + k*lambdaR*Isabx + 2*k*c*lambdaR*Issabx  
    dIssabx <- p_abx_10*Iss - gammaC*Issabx - (gammaS+w)*Issabx - 2*k*c*lambdaR*Issabx   
    dIrrabx <- p_abx_10*Irr - gammaC*Irrabx + lambdaXRR*Iabx - gammaR*Irrabx 
    ...
    dR <- -lambdaXS*R - lambdaXR*R - lambdaXSR*R - lambdaXSS*R - lambdaXRR*R + 
      ...
      tau*a_10*R + r*Ra + gammaC*I + r_abx*Rabx + 
      gammaR*Rrabx + gammaR*Rrrabx
    
    dRs <- lambdaXS*R - gammaS*Rs - k*lambdaS*Rs - k*lambdaR*Rs - tau*a_10*Rs + gammaC*Is + r_abx*Rsabx
    dRr <- lambdaXR*R - gammaR*Rr - k*lambdaS*Rr - k*lambdaR*Rr + (gammaS+w)*Rsra - tau*a_10*Rr + r*Rra + gammaC*Ir + r_abx*Rrabx
    
    dRsr <- lambdaXSR*R - gammaR*Rsr + k*lambdaR*Rs + k*lambdaS*Rr - 
      k*c*lambdaS*Rsr - k*c*lambdaR*Rsr + 2*k*c*lambdaR*Rss + 2*k*c*lambdaS*Rrr - 
      tau*a_10*Rsr + gammaC*Isr + r_abx*Rsrabx
    
    dRss <- lambdaXSS*R - gammaS*Rss + k*lambdaS*Rs + k*c*lambdaS*Rsr - 2*k*c*lambdaR*Rss - 
      tau*a_10*Rss + gammaC*Iss + r_abx*Rssabx
    
    dRrr <- lambdaXRR*R - gammaR*Rrr + k*lambdaR*Rr + k*c*lambdaR*Rsr - 2*k*c*lambdaS*Rrr - 
      tau*a_10*Rrr + r*Rrra + gammaC*Irr + r_abx*Rrrabx
    ...
    dATB <- tau*a_10*(S+Ss+Ssr+Ssr+Sss+Srr+E+Es+Er+Esr+Ess+Err) +
      (1-p_abx_10)*tau*a_10*(I+Is+Ir+Isr+Iss+Irr) + 
      p_abx_10*tau*a_10*(I+Is+Ir+Isr+Iss+Irr) + 
    ...
    return(list(c(dS, dSs, dSr, dSsr, dSss, dSrr, dSa, dSsa, dSra, dSsra, dSssa, dSrra,  
                  ...
                  dIPDs, dIPDr, dIPD, dATB), p_abx_10=p_abx_10,beta_10=thetabeta_10)) 
  })
...
R_prev_1000_10 <- round((out10$R_10[1000]/out10$PC_10[1000])*100, digits = 2)
R_prev_1000_10
# prevalence of the resistant carriage at the end of lockdown 1135 (%)
R_prev_1135_10 <- round((out10$R_10[1135]/out10$PC_10[1135])*100, digits = 2)
R_prev_1135_10
# prevalence of the resistant carriage at the end of the year 1365 (%)
R_prev_1365_10 <- round((out10$R_10[1365]/out10$PC_10[1365])*100, digits = 2)
R_prev_1365_10



plot(out10$R_10/out10$PC_10, type = "l")



#~~~~~~~~~~~~~~~~~~~~~~~~~~~ Change in the resistance prevalence ~~~~~~~~~~~~~~~~~~~~~~~~~~~
resistant_percent_change_lockdown_10 <- round(R_prev_1135_10 - R_prev_1000_10, digits = 2)
print('% change in the prevalence of resistant carriage at the end of the lockdown')
resistant_percent_change_lockdown_10

# change in the prevalence of RESISTANT bacterial carriage at the end of the YEAR
resistant_percent_change_10 <- round(R_prev_1365_10 - R_prev_1000_10, digits = 2)
print('% change in the prevalence of resistant carriage at the end of the year')
resistant_percent_change_10 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
p1 <- ggplot(output_10, aes(x=time)) + 
...
p2 <- ggplot(out10, aes(x=time)) + 
  geom_line(aes(y=R_10/PC_10, color="R"), size=1) +
  #geom_line(aes(y=(I+Is+Ir+Isr+Iss+Irr+Ia+Isa+Ira+Isra+Issa+Irra+Iabx+Isabx+Irabx+Israbx+Issabx+Irrabx)/N, color="covid"), size=1) +
  scale_x_continuous(limits = c(995, 1365), breaks=seq(1000, 1365, 60), labels = values1) +
  ...
  annotate(geom="text", x=1135, y=0.15, label=resistant_percent_change_lockdown_10, color="black", size = 8, fontface = 'bold') +
  annotate(geom="text", x=1185, y=0.15, label="%", color="black", size = 8, fontface = 'bold') +
  annotate(geom="text", x=1305, y=0.15, label=resistant_percent_change_10, color="black", size = 8, fontface = 'bold') +
...
relative_percent_change_10

# Daily values:
output_10$daily_IPDr <- diff(c(0, output_10$IPDr))               # true daily incidence of Virus 1


values1 <- c ("0", "60", "120", "180", "240", "300", "360")
# baseline resistant IPD incidence over one year
p3 <- ggplot(data=output_10, aes(x=time, y=daily_IPDr)) + 
...
################################################################################
total_short_10                 # 60-day IPD incidence
total_long_10                  # Annual IPD incidence
R_long_10                      # Annual resistant IPD incidence
AR_a10 <- (R_long_10/total_long_10)*100  
AR_a10 # ANTIBIOTIC RESISTANCE (%)
################################################################################
output_10$ATB[1365]-output_10$ATB[1000] 


library(cowplot)
Fig_4 <- plot_grid(p1, p2, p3, 

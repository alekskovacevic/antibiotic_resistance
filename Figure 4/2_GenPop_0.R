...
parameters_0 <- c(betaS = 0.042,  # pneumococcal transmission rate 0.062
                ...
                tau = 0.0014,     # antibiotic prescription rate (per capita), 
...
p_abx_0 = 0                    # proportion of infected individuals getting abx prophylaxis
psummer_0 = 1e-06              # summer invasion rate
pwinter_0 = 2.5e-06            # winter invasion rate 
a_lockdown_0 = 0.77            # reduction in the nb of abx prescriptions
covid_transm_lockdown_0 = 0.23 # reduction of the covid transmission during lockdown
IPD_risk_lockdown_0 = 0.2      # risk of IPD during lockdown 
IPD_risk_rest_0 = 0.4          # risk of IPD during the rest of the year 
covid.present_0 = 1            # presence(1)/absence(0) of covid (pre-pandemic and pandemic period)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
p_0 <- as.data.frame(list(times = times, p_inv_0 = rep(0, length(times))))
p_0$p_inv_0 <- ifelse((p_0$times > 1089 & p_0$times < 1271), (psummer_0), (pwinter_0)) 
invasion_0 <- approxfun(p_0)       #Create the interpolating function
invasion_0(seq(from = 1, to = 3650, by = 1))
...
atb_0$a_0 <- ifelse(atb_0$times > 1074 & atb_0$times < 1365, a_lockdown_0, 1)
input_0 <- approxfun(atb_0)
input_0(seq(from = 0, to = 3650, by = 1)) ##################### 
...
beta_0$thetabeta_0 <- ifelse(beta_0$times < 1075, 1,
                               ifelse((beta_0$times > 1074 & beta_0$times < 1136), 
                                      covid_transm_lockdown_0, 
                                      ifelse(beta_0$times > 1135 & beta_0$times < 1231, 0.45, 0.85))) 
inputbeta_0 <- approxfun(beta_0)
inputbeta_0(seq(from = 0, to = 3650, by = 1))

# Pneumococcal transmission risk during lockdown and the rest of the year 
beta_0p <- as.data.frame(list(times = times, thetab_0p = rep(0, length(times))))
beta_0p$thetab_0p <- ifelse((beta_0p$times > 1074 & beta_0p$times < 1136|beta_0p$times > 1304 & beta_0p$times < 1366), 1, 1) 
inputbeta_0p <- approxfun(beta_0p)
inputbeta_0p(seq(from = 0, to = 3650, by = 1))

# IPD disease risk reduction during NPI implementation before and during lockdown
risk_0 <- as.data.frame(list(times = times, IPDrisk_0 = rep(0, length(times))))
risk_0$IPDrisk_0 <- ifelse(risk_0$times < 1075, 1,
                           ifelse(risk_0$times > 1074 & risk_0$times < 1136, IPD_risk_lockdown_0, IPD_risk_rest_0)) 
inputrisk_0 <- approxfun(risk_0)
inputrisk_0(seq(from = 1, to = 3650, by = 1))
...
model_0 <- function(time, state, parameters) {  
  with(as.list(c(state, parameters)), { 
    covid = if(floor(time) == 1000) { covid = covid.present_0 } else {covid = 0}  # COVID
    
    inv_rate_0 <- invasion_0(time)       # invasion rate varies between summer and winter
    a_0 <- input_0(time)                   # antibiotic use in the community
    thetabeta_0 <- inputbeta_0(time)       # covid transmission risk 
    thetab_0 <- inputbeta_0p(time)            # carriage transmission risk
    IPDrisk_0 <- inputrisk_0(time)           # IPD risk (varies with the present or absence of ILIs)
    ...
    lambdaS <- betaS*thetab_0*(Ss + 2*q*ps*Ssr + 2*q*Sss + Ssa + 2*q*ps*Ssra + 2*q*Sssa + 
    ...
    lambdaR <- betaS*fit*thetab_0*(Sr + 2*q*(1-ps)*Ssr + 2*q*Srr + Sra + 2*q*(1-ps)*Ssra + 2*q*Srra +
    ...
    lambdaXS <- betaS*thetab_0*(Ss + psingle*(2*q*ps*Ssr + 2*q*Sss) + Ssa + psingle*(2*q*ps*Ssra + 2*q*Sssa) + 
    ...
    lambdaXR <- betaS*fit*thetab_0*(Sr + psingle*(2*q*(1-ps)*Ssr + 2*q*Srr) + Sra + psingle*(2*q*(1-ps)*Ssra + 2*q*Srra) + 
    ...
    lambdaXSR <- ((1-psingle)*(betaS*thetab_0*ps + betaS*thetab_0*fit*(1-ps))*2*q*(Ssr+Ssra) + 
                    (1-psingle)*(betaS*thetab_0*ps + betaS*thetab_0*fit*(1-ps))*2*q*(Esr+Esra) + 
                    (1-psingle)*(betaS*thetab_0*ps + betaS*thetab_0*fit*(1-ps))*2*q*(Isr+Isra+Israbx) +
                    (1-psingle)*(betaS*thetab_0*ps + betaS*thetab_0*fit*(1-ps))*2*q*(Rsr+Rsra+Rsrabx))/N
    
    lambdaXSS <- ((1-psingle)*betaS*thetab_0*2*q*(Sss+Sssa) + 
                    (1-psingle)*betaS*thetab_0*2*q*(Ess+Essa) + 
                    (1-psingle)*betaS*thetab_0*2*q*(Iss+Issa+Issabx) + 
                    (1-psingle)*betaS*thetab_0*2*q*(Rss+Rssa+Rssabx))/N
    
    lambdaXRR <- ((1-psingle)*betaS*thetab_0*fit*2*q*(Srr+Srra) + 
                    (1-psingle)*betaS*thetab_0*fit*2*q*(Err+Erra) + 
                    (1-psingle)*betaS*thetab_0*fit*2*q*(Irr+Irra+Irrabx) + 
                    (1-psingle)*betaS*thetab_0*fit*2*q*(Rrr+Rrra+Rrrabx))/N
    
    # Force of infection (SARS-CoV-2)
    lambdaC <- betaC*thetabeta_0*((I+Is+Ir+Isr+Iss+Irr+Ia+Isa+Ira+Isra+Issa+Irra+
    ...
    dS <- -lambdaXS*S - lambdaXR*S - lambdaXSR*S - lambdaXSS*S - lambdaXRR*S + 
      ...
      tau*a_0*S + r*Sa - lambdaC*S - covid  
    
    dSs <- lambdaXS*S - gammaS*Ss - k*lambdaS*Ss - k*lambdaR*Ss - tau*a_0*Ss - lambdaC*Ss  
    dSr <- lambdaXR*S - gammaR*Sr - k*lambdaS*Sr - k*lambdaR*Sr - tau*a_0*Sr + r*Sra + (gammaS+w)*Ssra - lambdaC*Sr 
    
    dSsr <- lambdaXSR*S - gammaR*Ssr + k*lambdaR*Ss + k*lambdaS*Sr - 
      k*c*lambdaS*Ssr - k*c*lambdaR*Ssr + 2*k*c*lambdaR*Sss + 2*k*c*lambdaS*Srr - 
      tau*a_0*Ssr - lambdaC*Ssr
    
    dSss <- lambdaXSS*S - gammaS*Sss + k*lambdaS*Ss + k*c*lambdaS*Ssr - 2*k*c*lambdaR*Sss - 
      tau*a_0*Sss - lambdaC*Sss
    
    dSrr <- lambdaXRR*S - gammaR*Srr + k*lambdaR*Sr + k*c*lambdaR*Ssr - 2*k*c*lambdaS*Srr - 
      tau*a_0*Srr + r*Srra - lambdaC*Srr
    
    # Antibiotic exposed compartments:
    dSa <- -lambdaXR*Sa - lambdaXRR*Sa + tau*a_0*S - r*Sa - lambdaC*Sa
    dSsa <- -(gammaS+w)*Ssa + tau*a_0*Ss - k*lambdaR*Ssa - lambdaC*Ssa
    dSra <- - gammaR*Sra + lambdaXR*Sa + tau*a_0*Sr - r*Sra - lambdaC*Sra
    dSsra <- -(gammaS+w)*Ssra + k*lambdaR*Ssa + 2*k*c*lambdaR*Sssa + tau*a_0*Ssr - lambdaC*Ssra
    dSssa <- -(gammaS+w)*Sssa - 2*k*c*lambdaR*Sssa + tau*a_0*Sss - lambdaC*Sssa
    dSrra <-  lambdaXRR*Sa + tau*a_0*Srr - r*Srra - gammaR*Srra - lambdaC*Srra
    ...
    dE <- -lambdaXS*E - lambdaXR*E - lambdaXSR*E - lambdaXSS*E - lambdaXRR*E + 
      ...
      tau*a_0*E + r*Ea + lambdaC*S - alpha*E   
    
    dEs <- lambdaXS*E - gammaS*Es - k*lambdaS*Es - k*lambdaR*Es - tau*a_0*Es + lambdaC*Ss - alpha*Es
    dEr <- lambdaXR*E - gammaR*Er - k*lambdaS*Er - k*lambdaR*Er - tau*a_0*Er + r*Era + lambdaC*Sr - alpha*Er + (gammaS+w)*Esra
    
    dEsr <- lambdaXSR*E - gammaR*Esr + k*lambdaR*Es + k*lambdaS*Er - 
      k*c*lambdaS*Esr - k*c*lambdaR*Esr + 2*k*c*lambdaR*Ess + 2*k*c*lambdaS*Err - 
      tau*a_0*Esr + lambdaC*Ssr - alpha*Esr 
    
    dEss <- lambdaXSS*E - gammaS*Ess + k*lambdaS*Es + k*c*lambdaS*Esr - 2*k*c*lambdaR*Ess - 
      tau*a_0*Ess + lambdaC*Sss - alpha*Ess
    
    dErr <- lambdaXRR*E - gammaR*Err + k*lambdaR*Er + k*c*lambdaR*Esr - 2*k*c*lambdaS*Err - 
      tau*a_0*Err + r*Erra + lambdaC*Srr - alpha*Err
    
    # Antibiotic exposed compartments:
    dEa <- -lambdaXR*Ea - lambdaXRR*Ea + tau*a_0*E - r*Ea + lambdaC*Sa - alpha*Ea
    dEsa <- -(gammaS+w)*Esa - k*lambdaR*Esa + tau*a_0*Es + lambdaC*Ssa - alpha*Esa
    dEra <- lambdaXR*Ea - gammaR*Era + tau*a_0*Er - r*Era + lambdaC*Sra - alpha*Era
    dEsra <- -(gammaS+w)*Esra + k*lambdaR*Esa + 2*k*c*lambdaR*Essa + tau*a_0*Esr + lambdaC*Ssra - alpha*Esra
    dEssa <- -(gammaS+w)*Essa - 2*k*c*lambdaR*Essa + tau*a_0*Ess + lambdaC*Sssa - alpha*Essa
    dErra <- lambdaXRR*Ea + tau*a_0*Err - r*Erra - gammaR*Erra + lambdaC*Srra - alpha*Erra
    ...
    dI <- -lambdaXS*I - lambdaXR*I - lambdaXSR*I - lambdaXSS*I - lambdaXRR*I + 
      ...
      (1-p_abx_0)*tau*a_0*I + r*Ia + alpha*E + covid - gammaC*I - p_abx_0*I + 
      gammaR*Irabx + gammaR*Irrabx
    
    dIs <- lambdaXS*I - gammaS*Is - k*lambdaS*Is - k*lambdaR*Is - (1-p_abx_0)*tau*a_0*Is + alpha*Es - gammaC*Is - p_abx_0*Is
    dIr <- lambdaXR*I - gammaR*Ir - k*lambdaS*Ir - k*lambdaR*Ir + (gammaS+w)*Isra - (1-p_abx_0)*tau*a_0*Ir + r*Ira + alpha*Er - gammaC*Ir - p_abx_0*Ir 
    
    dIsr <- lambdaXSR*I - gammaR*Isr + k*lambdaR*Is + k*lambdaS*Ir - 
      k*c*lambdaS*Isr - k*c*lambdaR*Isr + 2*k*c*lambdaR*Iss + 2*k*c*lambdaS*Irr - 
      (1-p_abx_0)*tau*a_0*Isr + alpha*Esr - gammaC*Isr - p_abx_0*Isr 
    
    dIss <- lambdaXSS*I - gammaS*Iss + k*lambdaS*Is + k*c*lambdaS*Isr - 2*k*c*lambdaR*Iss - 
      (1-p_abx_0)*tau*a_0*Iss + alpha*Ess - gammaC*Iss - p_abx_0*Iss 
    
    dIrr <- lambdaXRR*I - gammaR*Irr + k*lambdaR*Ir + k*c*lambdaR*Isr - 2*k*c*lambdaS*Irr - 
      (1-p_abx_0)*tau*a_0*Irr + r*Irra + alpha*Err - gammaC*Irr - p_abx_0*Irr 
    
    # Antibiotic exposed compartments:
    dIa <- -lambdaXR*Ia - lambdaXRR*Ia + (1-p_abx_0)*tau*a_0*I - r*Ia + alpha*Ea - gammaC*Ia 
    dIsa <- -(gammaS+w)*Isa - k*lambdaR*Isa + (1-p_abx_0)*tau*a_0*Is + alpha*Esa - gammaC*Isa
    dIra <- lambdaXR*Ia - gammaR*Ira + (1-p_abx_0)*tau*a_0*Ir - r*Ira + alpha*Era - gammaC*Ira
    dIsra <- -(gammaS+w)*Isra + k*lambdaR*Isa + 2*k*c*lambdaR*Issa + (1-p_abx_0)*tau*a_0*Isr + alpha*Esra - gammaC*Isra 
    dIssa <- -(gammaS+w)*Issa - 2*k*c*lambdaR*Issa + (1-p_abx_0)*tau*a_0*Iss + alpha*Essa - gammaC*Issa  
    dIrra <-  lambdaXRR*Ia + (1-p_abx_0)*tau*a_0*Irr - r*Irra - gammaR*Irra + alpha*Erra - gammaC*Irra 
    
    # Additional antibiotic (abx) exposed compartments:
    dIabx <- p_abx_0*I - gammaC*Iabx - lambdaXR*Iabx - lambdaXRR*Iabx + (gammaS+w)*Isabx + (gammaS+w)*Issabx 
    dIsabx <- p_abx_0*Is - gammaC*Isabx - (gammaS+w)*Isabx - k*lambdaR*Isabx   
    dIrabx <- p_abx_0*Ir - gammaC*Irabx + lambdaXR*Iabx - gammaR*Irabx + (gammaS+w)*Israbx 
    dIsrabx <- p_abx_0*Isr - gammaC*Israbx - (gammaS+w)*Israbx + k*lambdaR*Isabx + 2*k*c*lambdaR*Issabx  
    dIssabx <- p_abx_0*Iss - gammaC*Issabx - (gammaS+w)*Issabx - 2*k*c*lambdaR*Issabx   
    dIrrabx <- p_abx_0*Irr - gammaC*Irrabx + lambdaXRR*Iabx - gammaR*Irrabx 
    ...
    dR <- -lambdaXS*R - lambdaXR*R - lambdaXSR*R - lambdaXSS*R - lambdaXRR*R + 
      ...
      tau*a_0*R + r*Ra + gammaC*I + r_abx*Rabx + 
      gammaR*Rrabx + gammaR*Rrrabx
    
    dRs <- lambdaXS*R - gammaS*Rs - k*lambdaS*Rs - k*lambdaR*Rs - tau*a_0*Rs + gammaC*Is + r_abx*Rsabx
    dRr <- lambdaXR*R - gammaR*Rr - k*lambdaS*Rr - k*lambdaR*Rr + (gammaS+w)*Rsra - tau*a_0*Rr + r*Rra + gammaC*Ir + r_abx*Rrabx
    
    dRsr <- lambdaXSR*R - gammaR*Rsr + k*lambdaR*Rs + k*lambdaS*Rr - 
      k*c*lambdaS*Rsr - k*c*lambdaR*Rsr + 2*k*c*lambdaR*Rss + 2*k*c*lambdaS*Rrr - 
      tau*a_0*Rsr + gammaC*Isr + r_abx*Rsrabx
    
    dRss <- lambdaXSS*R - gammaS*Rss + k*lambdaS*Rs + k*c*lambdaS*Rsr - 2*k*c*lambdaR*Rss - 
      tau*a_0*Rss + gammaC*Iss + r_abx*Rssabx
    
    dRrr <- lambdaXRR*R - gammaR*Rrr + k*lambdaR*Rr + k*c*lambdaR*Rsr - 2*k*c*lambdaS*Rrr - 
      tau*a_0*Rrr + r*Rrra + gammaC*Irr + r_abx*Rrrabx
    
    # Antibiotic exposed compartments:
    dRa <- gammaC*Ia + tau*a_0*R - r*Ra - lambdaXR*Ra - lambdaXRR*Ra 
    dRsa <- gammaC*Isa + tau*a_0*Rs - (gammaS+w)*Rsa - k*lambdaR*Rsa
    dRra <- gammaC*Ira + tau*a_0*Rr - r*Rra + lambdaXR*Ra - gammaR*Rra
    dRsra <- gammaC*Isra + tau*a_0*Rsr - (gammaS+w)*Rsra + k*lambdaR*Rsa + 2*k*c*lambdaR*Rssa 
    dRssa <- gammaC*Issa + tau*a_0*Rss - (gammaS+w)*Rssa - 2*k*c*lambdaR*Rssa
    dRrra <- gammaC*Irra + tau*a_0*Rrr - r*Rrra + lambdaXRR*Ra - gammaR*Rrra 
    ...
    dIPDs <- inv_rate_0*IPDrisk_0*(Ss+Sss+0.5*Ssr+Es+Ess+0.5*Esr+Is+Iss+0.5*Isr+Rs+Rss+0.5*Rsr)
    
    # IPD caused by R can be caused by all individuals carrying R:
    dIPDr <- inv_rate_0*IPDrisk_0*(Sr+Srr+0.5*Ssr+Er+Err+0.5*Esr+Ir+Irr+0.5*Isr+Rr+Rrr+0.5*Rsr+
                                     Sra+Srra+Ssra+Era+Erra+Esra+Ira+Irra+Isra+Rra+Rrra+Rsra+
                                     Irabx+Irrabx+Israbx+Rrabx+Rrrabx+Rsrabx)
    
    dIPD <- inv_rate_0*IPDrisk_0*(Ss+Sss+0.5*Ssr+Es+Ess+0.5*Esr+Is+Iss+0.5*Isr+Rs+Rss+0.5*Rsr)+
      inv_rate_0*IPDrisk_0*(Sr+Srr+0.5*Ssr+Er+Err+0.5*Esr+Ir+Irr+0.5*Isr+Rr+Rrr+0.5*Rsr+
                              Sra+Srra+Ssra+Era+Erra+Esra+Ira+Irra+Isra+Rra+Rrra+Rsra+
                              Irabx+Irrabx+Israbx+Rrabx+Rrrabx+Rsrabx)
    
    # Cumulative number of antibiotic exposed individuals:
    dATB <- tau*a_0*(S+Ss+Ssr+Ssr+Sss+Srr+E+Es+Er+Esr+Ess+Err) +
      (1-p_abx_0)*tau*a_0*(I+Is+Ir+Isr+Iss+Irr) + 
      p_abx_0*tau*a_0*(I+Is+Ir+Isr+Iss+Irr) + 
      tau*a_0*(R+Rs+Rr+Rsr+Rss+Rrr) 
    ...
    return(list(c(dS, dSs, dSr, dSsr, dSss, dSrr, dSa, dSsa, dSra, dSsra, dSssa, dSrra,  
                  ...
                  dIPDs, dIPDr, dIPD, dATB), p_abx_0=p_abx_0,beta_0=thetabeta_0)) 
  })
  
}

# Solving the differential equations using the ode integration algorithm
output_0 <- as.data.frame(ode(y = initial_state, 
                               times = sim_time, 
                               func = model_0, 
                               parms = parameters_0, 
                               method = "euler"))
...
out0 <- dplyr::mutate(output_0, PC_0 = Ss + Sr + Ssr + Sss + Srr + Ssa + Sra + Ssra + Sssa + Srra + 
                      ...
                      R_0 = Sr + 0.5*Ssr + Srr + Sra + 0.5*Ssra + Srra + 
                      ...
                      S_0 = Ss + 0.5*Ssr + Sss + Ssa + 0.5*Ssra + Sssa + 
...
R_prev_1000_0 <- round((out0$R_0[1000]/out0$PC_0[1000])*100, digits = 2)
R_prev_1000_0
# prevalence of the resistant carriage at the end of lockdown 1135 (%)
R_prev_1135_0 <- round((out0$R_0[1135]/out0$PC_0[1135])*100, digits = 2)
R_prev_1135_0
# prevalence of the resistant carriage at the end of the year 1365 (%)
R_prev_1365_0 <- round((out0$R_0[1365]/out0$PC_0[1365])*100, digits = 2)
R_prev_1365_0



plot(out0$R_0/out0$PC_0, type = "l")



#~~~~~~~~~~~~~~~~~~~~~~~~~~~ Change in the resistance prevalence ~~~~~~~~~~~~~~~~~~~~~~~~~~~
resistant_percent_change_lockdown_0 <- round(R_prev_1135_0 - R_prev_1000_0, digits = 2)
print('% change in the prevalence of resistant carriage at the end of the lockdown')
resistant_percent_change_lockdown_0

# change in the prevalence of RESISTANT bacterial carriage at the end of the YEAR
resistant_percent_change_0 <- round(R_prev_1365_0 - R_prev_1000_0, digits = 2)
print('% change in the prevalence of resistant carriage at the end of the year')
resistant_percent_change_0 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
p2 <- ggplot(out0, aes(x=time)) + 
  geom_line(aes(y=R_0/PC_0, color="R"), size=1) +
  geom_line(aes(y=(I+Is+Ir+Isr+Iss+Irr+Ia+Isa+Ira+Isra+Issa+Irra+Iabx+Isabx+Irabx+Israbx+Issabx+Irrabx)/N, color="covid"), size=1) +
  scale_x_continuous(limits = c(995, 1365), breaks=seq(1000, 1365, 60), labels = values1) +
  ...
  annotate(geom="text", x=1135, y=0.15, label=resistant_percent_change_lockdown_0, color="black", size = 8, fontface = 'bold') +
  annotate(geom="text", x=1185, y=0.15, label="%", color="black", size = 8, fontface = 'bold') +
  annotate(geom="text", x=1305, y=0.15, label=resistant_percent_change_0, color="black", size = 8, fontface = 'bold') +
...
total_short_0 <- output_0$IPD[1135] - output_0$IPD[1075] # per 100,000 people cumulative ipd incidence during the 60-day period
total_long_0 <- output_0$IPD[1365] - output_0$IPD[1000] # per 100,000 people cumulative ipd incidence during one year since the outbreak

# IPD incidence caused by susceptible strain only (S + 0.5*SR):
# short term
S_short_0 <- output_0$IPDs[1135] - output_0$IPDs[1075] # per 100,000 people cumulative ipd incidence during the 60-day period
S_long_0 <- output_0$IPDs[1365] - output_0$IPDs[1000] # per 100,000 people cumulative ipd incidence during one year since the outbreak

# IPD incidence caused by resistant strain only (R + 0.5*SR):
# short term
R_short_0 <- output_0$IPDr[1135] - output_0$IPDr[1075] # per 100,000 people cumulative ipd incidence during the 60-day period
R_long_0 <- output_0$IPDr[1365] - output_0$IPDr[1000] # per 100,000 people cumulative ipd incidence during one year since the outbreak


# change in the prevalence of TOTAL bacterial carriage at the end of the 60-day lockdown period
relative_percent_change_0 <- ((out0$PC_0[1135] - out0$PC_0[1075])/out0$PC_0[1075])*100
print('% change in the prevalence of TOTAL pneumococcal carriage at the end of the 60-day period')
relative_percent_change_0

# Daily values:
output_0$daily_IPDr <- diff(c(0, output_0$IPDr))               # true daily incidence of Virus 1


values1 <- c ("0", "60", "120", "180", "240", "300", "360")
# baseline resistant IPD incidence over one year
p3 <- ggplot(data=output_0, aes(x=time, y=daily_IPDr)) + 
...
################################################################################
total_short_0                 # 60-day IPD incidence
total_long_0                  # Annual IPD incidence
R_long_0                      # Annual resistant IPD incidence
AR_a0 <- (R_long_0/total_long_0)*100
AR_a0 # ANTIBIOTIC RESISTANCE (%)
################################################################################
output_0$ATB[1365]-output_0$ATB[1000]



library(cowplot)
Fig_2 <- plot_grid(p1, p2, p3, 
